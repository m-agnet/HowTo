...
以下の雛形をコピー&ペーストで任意のtexファイルに書き込む. (拡張子は「.tex」, ファイル名に日本語を使うとコンパイルがうまくいかないことがあるので注意.)
...
ホームディレクトリの隠しファイル「.latexmkrc」に以下を書き込む. 
...
「管理->設定->JSONを開く(右上にあるアイコン)」で以下のコードをコピー&ペーストする.
