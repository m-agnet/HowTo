...
# パラメータの範囲を設定
Ay_range = range(10, 20, length=2) # Ayの範囲
...
Ra_range = range(0.938769, length=1) # 濡れ具合の範囲
run_range = range(1e6, length=1) # 実行ステップ数の範囲
...
for Ay_value in Ay_range, 
    ...
    # LAMMPSを実行
    run(`myqsub -Q ness -C 4 -N Rt$(Rt_value)Ra$(Ra_value) mpirun -n 4 lmp_mpi -log $(outputtitle).log -in $(tempfile)`) 
